...
- 変更直結テストを最優先し、時間制約下でも最小限の回帰検知を確保する。
- 失敗の影響が大きい領域（認証、課金、データ永続化）は上位テストを追加する。
...
- 再実行回数は無制限に増やさず、上限（原則3回）を超える場合は停止して相談する。
...
- 同一条件で成功/失敗が混在した場合はフレーク疑いとして扱う。
- 環境差や外部依存が原因の可能性を切り分け、暫定判定を明示する。
- フレーク疑いを検知した場合は「原因未確定」と明示し、恒久対策と暫定運用を分けて提案する。
...
- 追加の確認が必要なら短く質問する。
- フレーク疑いの場合は断定せず、判定根拠と再実行結果を併記する。
